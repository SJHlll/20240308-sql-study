...
*/
SELECT * FROM employees
WHERE manager_id = (SELECT manager_id FROM departments WHERE manager_id = 100);
...
SELECT
    ...
    d.department_id AS 부서아이디, d.department_name AS 부서명
FROM employees e
...
*/
SELECT
    e.employee_id AS 직원아이디, e.first_name || ' ' || e.last_name AS 이름, e.department_id AS 부서아이디,
    (
    SELECT 
        d.department_name
    FROM departments d
