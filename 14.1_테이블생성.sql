...
INSERT INTO dept2
VALUES(30, '경영지원', '경기', sysdate, 500000);
...
-- 컬럼 속성 수정
-- 만약 변경하고자 하는 컬럼에 데이터가 이미 존재한다면 그에 맞는 타입으로 변경해야 한다
-- 안맞는 타입으로는 변경이 불가능하다
ALTER TABLE dept2
MODIFY dept_name VARCHAR2(50);

-- DDL(CREATE, ALTER, TRUNCATE, DROP)은 트랜잭션의 대상이 아니다
ROLLBACK;
