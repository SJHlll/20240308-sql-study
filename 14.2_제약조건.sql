...
UPDATE dept2
SET loca = 4000
WHERE dept_no = 10; -- 실패(왜래키 제약조건 위반)
...
-- pk 추가
ALTER TABLE dept2 ADD CONSTRAINT dept2_deptno_pk
PRIMARY KEY(dept_no);
-- fk 추가
...
CHECK(dept_bonus > 10000);
-- unique 추가
ALTER TABLE dept2 ADD CONSTRAINT dept2_depname_uk
UNIQUE(dept_name);
-- NOT NULL은 열 수정형태로 변경된다
ALTER TABLE dept2 MODIFY dept_bonus NUMBER(10) NOT NULL;

-- 제약 조건 확인
SELECT * FROM user_constraints
WHERE table_name = 'DEFT2';
...
CREATE TABLE members(
    ...
    m_num NUMBER(3) CONSTRAINT mem_mexnum_pk PRIMARY KEY,
    reg_date DATE NOT NULL CONSTRAINT mam_regdate_uk UNIQUE,
    gender VARCHAR2(1),
    loca NUMBER(4) CONSTRAINT mem_loca_loc_lucid_fk REFERENCES locations(location_id)
);
