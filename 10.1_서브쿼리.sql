...
-- 이름이 'Nancy'인 사원보다 급여가 높은 사원을 찾기
SELECT salary
FROM employees
...
-- 다음 문장은 서브쿼리가 리턴하는 뱅이 여러개라서 단일행 연산자를 사용할 수 없다
-- 단일 행 연산자 : 주로 비교 연산자(=, >, >=, <, <=, <>)를 사용하는 경우 하나의 행만 반환해야 한다
-- 이런 경우 다중 행 연산자를 사용해야 한다
SELECT * FROM employees
WHERE job_id = (SELECT job_id FROM employees
                WHERE last_name = 'King'); -- 에러

-- 다중 행 연산자(IN, ANY, ALL, EXISTS)
-- IN : 조회된 목록의 어떤 값과 같은지를 비교한다
...
-- DAVID라는 사람이 여러 명인데, 그 중에 가장 작은 급여를 받는 David보다 급여가 높은 사람을 조회
SELECT * FROM employees
WHERE salary > ANY (SELECT salary FROM employees
                WHERE first_name = 'David');

-- ALL : 값을 서브쿼리에 의해 리턴된 각각의 값과 모두 비교해서
-- 전부 다 일치해야 조회대상에 포함된다.
...
-- SELECT 절에 서브쿼리를 붙이기
-- 스칼리 서브쿼리 라고도 칭한다
-- 스칼라 서브쿼리 : 실행결과가 단일 값을 반환하는 서브쿼리. 주로 SELECT절이나 WHERE절에서 사용함
