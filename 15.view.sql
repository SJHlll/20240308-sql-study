...
-- 뷰의 컬럼명은 함수 호출문, 연산식 등 같은 기상 표현식이면 안된다
...
SELECT * FROM view_map
WHERE salary >= 000;


-- 복합 뷰
-- 여러 테이블을 조인하여 필요한 데이터만 저장함
CREATE VIEW view_map_dept_job AS (
    SELECT
        e.employee_id,
...
SELECT * FROM view_map_dept_job;

-- 뷰의 수정 (CREATE OR REPLACE VIEW 구문)
-- 동일 이름으로 해당 구문을 사용하면 데이터가 변경되면서 새롭게 생성된다
CREATE OR REPLACE VIEW view_map_dept_job AS (
    SELECT
        e.employee_id,
        e.first_name || ' ' || last_name AS full_name,
        d.department_name,
...
-- 두번째 컬럼임 'full_name'은 가상열(virtual column)이기 때문에 INSERT가 안됨
INSERT INTO view_map_dept_job
...
-- 원본 테이블 컬럼 중 NOT NULL 컬럼이 존재하고, 해당 컬럼을 view로 지목할 수 없다면 INSERT가 불가능하다
INSERT INTO view_map
    (employee_id, job_id, salary)
VALUES(300, 'test', 10000);

DELETE FROM view_map
...
-- 읽기 전용 뷰 -> WITH READ ONLY (DML 연산을 막음 -> SELECT만 막음)
CREATE OR REPLACE VIEW view_emp_test AS (
    SELECT
        employee_id,
        first_name,
